...
bool LinkedHashMap<KEY, VALUE>::pushBack(
        const KEY&   key,
        const VALUE& value)
{
    Guard lock{mutex};
...
template<class KEY, class VALUE>
VALUE& LinkedHashMap<KEY, VALUE>::front() noexcept
{
    Guard lock{mutex};
    if (map.size() == 0)
...
template<class KEY, class VALUE>
void LinkedHashMap<KEY, VALUE>::pop() noexcept
{
    Guard lock{mutex};
    if (map.size() == 0)
        throw OUT_OF_RANGE("Map is empty");
...
template<class KEY, class VALUE>
bool LinkedHashMap<KEY, VALUE>::remove(KEY& key) noexcept
{
    Guard lock{mutex};
